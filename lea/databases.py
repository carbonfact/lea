...
class BigQueryJob:
    ...
    def n_rows_in_destination(self) -> int | None:
        if self.query_job.destination is None:
            raise ValueError("Job has no destination")
        if self.client.dry_run:
            return None
        return self.client.client.get_table(self.query_job.destination).num_rows
...
class BigQueryClient:
    ...
    def clone_table(self, from_table_ref: scripts.TableRef, to_table_ref: scripts.TableRef) -> BigQueryJob:
        ...
        """
        job_config = self.make_job_config()
        return BigQueryJob(client=self, query_job=self.client.query(clone_code, job_config=job_config))
